...
func pidFilePath() string  { return filepath.Join(".", pidFileName) }
func killFilePath() string { return filepath.Join(".", killFileName) }
...
func main() {
    ...
    case "start":
        ...
        for i := 2; i < len(os.Args); i++ {
            switch os.Args[i] {
            case "-fps":
                i++; fps, _ = strconv.Atoi(os.Args[i])
                reexecArgs = append(reexecArgs, "-fps", os.Args[i])
            case "-duration":
                i++; duration, _ = strconv.Atoi(os.Args[i])
